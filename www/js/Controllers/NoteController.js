...
.controller('NoteCtrl',function($scope, $localStorage, $rootScope,  $http, $location, $stateParams, $ionicPopup){
	...
	$scope.init = function(){
		$scope.initBypass = true;
		$http.get('http://62.210.115.66:9000/getGrade/'+$localStorage.getObject('user').id+'/'+$scope.friend.id).success(function(response){
	...
	$scope.postNote = function(){
		...
		else
		{
			...
			$http.post('http://62.210.115.66:9000/Notation/grade',{noteur: $localStorage.getObject('user').id, note: $scope.friend.id, technique:$scope.notes[0],frappe:$scope.notes[1],physique:$scope.notes[2],fair_play:$scope.notes[3],assiduite:$scope.notes[4] }).success(function(){
				for(var i=0; i<5; i++) {
